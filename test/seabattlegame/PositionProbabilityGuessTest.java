...
public class PositionProbabilityGuessTest {
    private PlayingField pf;
    private GameConf gameConf;
    private PositionProbabilityGuess1 ppg;
//    private CellState playingFieldState[][];
    public static void main(String[] args){
        PositionProbabilityGuessTest test=new PositionProbabilityGuessTest();
        test.test();
    }
...
//        gameConf= new GameConf(10,10, new ArrayList<>(Arrays.asList(new Point[] {new Point(4,1)})));
        pf=new PlayingField(gameConf);
        pf.initialize();
//        playingFieldState=pf.getPlayingFieldState();
        ppg=new PositionProbabilityGuess1(pf);
//        playingFieldState[5][3]=CellState.HITTEDDECK;
//        print_pfs();
        ppg.obtainGuess();
...
//    private void print_pfs() {
//        for (int i=0; i<gameConf.getHeight();i++){
//            for(int j=0;j<gameConf.getWidth();j++){
//                System.out.print(playingFieldState[i][j]+" ");
//            }
//            System.out.println();
//        }
//    }
...
class PositionProbabilityGuess1 extends RandomGuess{
...
//    private final int height;
    ...
    PositionProbabilityGuess1 (PlayingField playingField){
        super(playingField);
...
//    }
    ...
    private class PositionProbabilityGuessArray{
        ...
        private int searchingShipIndex=0;
...
//            pfc=new ProbabilityForCell [shipTypeCount+2][width*height]; //shipTypeCount cуммарная матрица
        ...
        private Point getGuess() {
            ...
            calculatePfcs();
//            for (int i=0;i<shipTypeCount-1;i++){
//                if (i!=searchingShipIndex) continue;
...
//            return pfc[shipTypeCount][(int) (Math.random() * lastIndexMaxProbability)].getCoordinates();
        ...
        private Point getRandMaxProbCell(ProbabilityForCell [] pfc){
            ...
            if (lastIndexMaxProbability==0)return null;
            else return pfc[(int) (Math.random() * lastIndexMaxProbability)].getCoordinates();
        ...
        private boolean doCalculate(int i){
            ...
            if (aliveShipTypes.get(i).x!=curDeckCount) return false; //только искомый корабль
...
//                if (aliveShipTypes.get(i).y>0){
//                    if(i==searchingShipIndex) break;
//                    else{
//                        searchingShipIndex=i;
//                        createMask();
//                        break;
//                    }
//                }
...
//        }
        ...
        private void calculateSumPfcForSearchingShip() {
            calculateSumPfcForShip(searchingShipIndex);
//            for (int i=0;i<pfc[shipTypeCount].length;i++)
...
//            }
            ...
            for(int i=0;i<mask.length;i++){
                mask[i].setProbability((int) Math.signum(pfc[searchingShipIndex][i].getProbability()));
            }
